...
/// LRU index for tracking access order and per-entry sizes
#[derive(Debug, Clone)]
struct LruIndex {
...
impl LruIndex {
    ...
    /// Add new entry with size tracking
    fn insert(&mut self, key: &str, size_bytes: u64) {
        // Remove old entry if it exists (to update size)
    ...
    /// Remove entry with proper size tracking
...
impl MediaCache {
    ...
    fn enforce_size_limit(&self) -> MediaResult<()> {
        ...
        // Use LRU's tracked size instead of iterating the database
        let current_bytes = self.lru.read().total_size();
        ...
        let bytes_to_free = current_bytes.saturating_sub(target_bytes);
    ...
    pub fn clear(&self) -> MediaResult<()> {
...

        {
            ...
            // Delete all entries using drain
            let _ = table.drain(..)
                .map_err(|e| MediaError::new(MediaErrorCode::CacheClearFailed, e.to_string()))?;
        }
    ...
    pub fn stats(&self) -> CacheStats {
        ...
        // Use LRU's tracked size (O(1)) instead of iterating all entries (O(N))
        let approx_size_bytes = self.lru.read().total_size();
