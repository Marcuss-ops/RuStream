...
pub fn probe_full(path: &str) -> MediaResult<FullMetadata> {
    ...
    let mut audio_metadata: Option<AudioMetadata> = None;
    ...
    for stream in context.streams() {
        ...
        match codec_type {
            ff::media::Type::Video => {
                ...
                // Note: Getting width/height requires opening codec
                // For lightweight probe, we skip this
            }
            ff::media::Type::Audio => {
                audio_metadata = Some(AudioMetadata {
                    codec: codec_name,
                    sample_rate: 0,  // Would need to open codec
                    channels: 0,
                    bit_depth: None,
                    bit_rate: None,
                });
